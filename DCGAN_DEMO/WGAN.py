...
G_optimizer = torch.torch.optim.RMSprop(G.parameters(),lr = 0.00005)
D_optimizer = torch.torch.optim.RMSprop(D.parameters(),lr = 0.00005)
...
for i in range(epoch):
    ...
    for T,img in enumerate(dataloader):
        ...
        fake_imgs = G(idea).detach()
        fake_pred = D(fake_imgs)
        dvalue_fake = torch.mean(fake_pred)
        dvalue = -(dvalue_real-dvalue_fake)
        dvalue.backward()
        D_optimizer.step()
